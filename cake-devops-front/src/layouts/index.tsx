...
const Layout: React.FC<LayoutProps> = ({dispatch, isLogin, userData}) => {
    ...
    const queryUserMenu = async () => {
        try {
            ...
            if (response.success) {
                ...
                const convertedMenu = convertMenuTreeToProLayoutMenu(menuTree);
    ...
    const convertMenuTreeToProLayoutMenu = (menuTree: MenuTreeDTO[]): MenuItem[] => {
        return menuTree.map(item => ({
            path: item.path,
            name: item.name,
            icon: item.icon ? <img src={item.icon} alt={`${item.name} icon`}/> : undefined,
            children: item.children ? convertMenuTreeToProLayoutMenu(item.children) : undefined,
        }));
    };
        ...
        >
            <ProLayout
                ...
                menu={{
                    ...
                    request: async () => menuData, // 使用 request 属性传递菜单数据
                }}
                waterMarkProps={{
